...
if(! isset($token)) {
		...
		/* The servername and serverport tells PayPal where the buyer
		  ...
		  $currencyCodeType=$_REQUEST['currencyCodeType'];
			...
			$sql="update user set address='".$_POST["address"]."',city='".$_POST["city"]."',state='".$_POST["state"]."',country='".$_POST["country"]."',pincode='".$_POST["zip"]."',Ship_Address='".$_REQUEST['SHIPTOSTREET']."',Ship_City='".$_REQUEST['SHIPTOCITY']."', Ship_State='".$_REQUEST['SHIPTOSTATE']."',Ship_Country='".$_REQUEST['SHIPTOCOUNTRYCODE']."',Ship_Zip='".$_REQUEST['SHIPTOZIP']."' where email= '".$_SESSION["email"]."'";
		...
		$SHIPTOZIP         = $_REQUEST['SHIPTOZIP'];
			...
			$totalitemamt = 0;
			//$Query .= "&L_NAME0=".$_REQUEST['L_NAME0']."&L_AMT0=".$_REQUEST['L_AMT0']."&L_QTY0=".$_REQUEST['L_QTY0']."&";
		  for($i=0;$i<$count;$i++){
		    /*$L_NAME=$_REQUEST['L_NAME'.$i];
		 	$L_AMT=$_REQUEST['L_AMT'.$i];
		 	$L_QTY=$_REQUEST['L_QTY'.$i];*/
			$totalitemamt = $totalitemamt+($_REQUEST['L_QTY'.$i]*$_REQUEST['L_AMT'.$i]);
			$Query .= "&L_NAME".$i."=".$_REQUEST['L_NAME'.$i]."&L_AMT".$i."=".$_REQUEST['L_AMT'.$i]."&L_QTY".$i."=".$_REQUEST['L_QTY'.$i]."&L_NUMBER".$i."=".$_REQUEST['L_ITEM'.$i]."&L_DESC".$i."=".$_REQUEST['L_SZ'.$i]."&";
		  }
		 //echo  $Query;exit;
		 ...
		 /* The returnURL is the location where buyers return when a
		   ...
		   $returnURL =urlencode($url.'/index.php?p=ReviewOrder&currencyCodeType='.$currencyCodeType.'&paymentType='.$paymentType);
		   $cancelURL =urlencode("$url/index.php?p=SetExpressCheckout&paymentType=$paymentType" );
		 ...
		 /* Construct the parameter string that describes the PayPal payment
          ...
          $shiptoAddress = "&SHIPTONAME=$personName&SHIPTOSTREET=$SHIPTOSTREET&SHIPTOCITY=$SHIPTOCITY&SHIPTOSTATE=$SHIPTOSTATE&SHIPTOCOUNTRYCODE=$SHIPTOCOUNTRYCODE&SHIPTOZIP=$SHIPTOZIP";
           ...
           $nvpstr="&ADDRESSOVERRIDE=1".$shiptoAddress."".$Query."MAXAMT=".(string)$maxamt."&AMT=".(string)$amt."&ITEMAMT=".(string)$totalitemamt."&CALLBACKTIMEOUT=4&ReturnUrl=".$returnURL."&CANCELURL=".$cancelURL ."&CURRENCYCODE=".$currencyCodeType."&PAYMENTACTION=".$paymentType;
