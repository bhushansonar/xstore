...
if ($a=="edit") {
	if ($DesignId!=0) {
		$SQL="select * from product_design where DesignId =".$DesignId;
		$rsUser = $objDB->select($SQL);
		if (count($rsUser)>0) {
			$DesignId = $rsUser[0]["DesignId"];
			$DesignName = $rsUser[0]["DesignName"];
				...
				<div class="hastable">
                	<?php if(isset($_SESSION['success']) && $_SESSION['success'] != '' && $_SESSION['check'] == 'add'){ ?>
                      ...
                      <?php if(!empty($rsUser)){ ?>
						...
						<tr>
							<td class="center"><input name="multipledel[]" type="checkbox" id="multipledel[]" value="<?php echo $rsUser[$i]['DesignId']?>"></td> 
                            <td><?php echo viewtext($rsUser[$i]['DesignName']);?></td>
                            
                            
                           <td><a href="<?php echo $AbsoluteURLAdmin;?>index.php?p=design&a=edit&DesignId=<?php echo $rsUser[$i]['DesignId']?>" title="Edit" style="float:left" class="btn_no_text btn ui-state-default ui-corner-all tooltip"><span class="ui-icon ui-icon-wrench"></span></a>
								...
								<script language="javascript" type="text/javascript">
                                    jQuery(document).ready(function() {
                                        jQuery("#modal_confirmation<?php echo $i?>").dialog({
                                            ...
                                            buttons: {
                                                'Delete': function() {
                                                    document.location.href="<?php echo $AbsoluteURLAdmin;?>manage.php?p=design&a=delete&DesignId=<?php echo $rsUser[$i]['DesignId']?>";
                                                    jQuery(this).dialog('close');
                                                },
                                                Cancel: function() {
                                                    jQuery(this).dialog('close');
                     ...
                     <input type="button" class="ui-button float-right ui-state-default ui-corner-all" id="create-user" value="Add New Design" onclick="document.location.href='index.php?p=design&a=add'" >
               		<div class="clear"></div>
				</div>
				<div class="clear"></div>
			</div>
...
$().ready(function() {
	...
	$("#FrmTestimonials").validate({
		...
		messages: {
			DesignName: "Please enter your DesignName"
		}
	}
	});
});
                    ...
                    <input type="hidden" name="p" value="design">
                    	...
                    	<ul>
                        	<li>
                                ...
                                <input type="text" name="DesignName" id="DesignName"  class="field text small"  value="<?php echo viewtext($DesignName);?>" />
