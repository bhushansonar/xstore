...
if ($a=="list") {
	...
	if($searchName != ''){
		$SQL .= "(firstname LIKE '%".$searchName."%' OR lastname LIKE '%".$searchName."%') and ";		
	}
	$SQL .= " 1=1 order by UserId ASC";
	...
	<div id="page-content">
			<div id="page-content-wrapper">
				...
				<div class="hastable">
						...
						<tr>
							<td class="center"><?php if ($_SESSION['session_adminID']!=$rsAdmin[$i]['UserId']) { ?><input name="multipledel[]" type="checkbox" id="multipledel[]" value="<?=$rsAdmin[$i]['UserId']?>"><?php }else{ ?>&nbsp;<?php } ?></td> 
                            ...
                            <td><?=ucwords($rsAdmin[$i]['LastLogin'])?></td>
							
							<td>
                            <a href="<?php echo $AbsoluteURLAdmin;?>index.php?p=user&a=edit&UserId=<?=$rsAdmin[$i]['UserId']?>" title="Edit" style="float:left" class="btn_no_text btn ui-state-default ui-corner-all tooltip"><span class="ui-icon ui-icon-wrench"></span></a>
                                         <?php if ($_SESSION['session_adminID']!=$rsAdmin[$i]['UserId']) { ?>
                                         ...
                                         <?php if($rsAdmin[$i]['Status'] == '1'){?>
										 	<a href="<?php echo $AbsoluteURLAdmin;?>manage.php?p=user&a=status&s=0&UserId=<?=$rsAdmin[$i]['UserId']?>" title="Deactivate" class="btn_no_text btn ui-state-default ui-corner-all tooltip"><span class="ui-icon ui-icon-radio-on"></span></a>
                                         <?php }else{ ?>
                                         	<a href="<?php echo $AbsoluteURLAdmin;?>manage.php?p=user&a=status&s=1&UserId=<?=$rsAdmin[$i]['UserId']?>" title="Activate" class="btn_no_text btn ui-state-default ui-corner-all tooltip"><span class="ui-icon ui-icon-bullet"></span></a>
                                         <?php } ?>
                                        ...
                                        <script language="javascript" type="text/javascript">
											jQuery(document).ready(function() {
												jQuery("#modal_confirmation<?=$i?>").dialog({
													...
													buttons: {
														'Delete': function() {
															document.location.href="<?php echo $AbsoluteURLAdmin;?>manage.php?p=user&a=delete&UserId=<?=$rsAdmin[$i]['UserId']?>";
                ...
                <input type="button" class="ui-button float-right ui-state-default ui-corner-all" id="create-user" value="Create New User" onclick="document.location.href='index.php?p=user'" >
					<div class="clear"></div>
				</div>
				<div class="clear"></div>
			</div>
