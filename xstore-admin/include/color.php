...
if ($a=="edit") {
	if ($ColorId!=0) {
		$SQL="select * from product_color where ColorId =".$ColorId;
		$rsUser = $objDB->select($SQL);
		if (count($rsUser)>0) {
			$ColorId = $rsUser[0]["ColorId"];
			$ColorName = $rsUser[0]["ColorName"];
				...
				<div class="hastable">
                	<?php if(isset($_SESSION['success']) && $_SESSION['success'] != '' && $_SESSION['check'] == 'add'){ ?>
                      ...
                      <?php if(!empty($rsUser)){ ?>
						...
						<tr>
							<td class="center"><input name="multipledel[]" type="checkbox" id="multipledel[]" value="<?php echo $rsUser[$i]['ColorId']?>"></td> 
                            <td><?php echo viewtext($rsUser[$i]['ColorName']);?></td>
                            
                            
                           <td><a href="<?php echo $AbsoluteURLAdmin;?>index.php?p=color&a=edit&ColorId=<?php echo $rsUser[$i]['ColorId']?>" title="Edit" style="float:left" class="btn_no_text btn ui-state-default ui-corner-all tooltip"><span class="ui-icon ui-icon-wrench"></span></a>
								...
								<script language="javascript" type="text/javascript">
                                    jQuery(document).ready(function() {
                                        jQuery("#modal_confirmation<?php echo $i?>").dialog({
                                            ...
                                            buttons: {
                                                'Delete': function() {
                                                    document.location.href="<?php echo $AbsoluteURLAdmin;?>manage.php?p=color&a=delete&ColorId=<?php echo $rsUser[$i]['ColorId']?>";
                                                    jQuery(this).dialog('close');
                                                },
                                                Cancel: function() {
                                                    jQuery(this).dialog('close');
                     ...
                     <input type="button" class="ui-button float-right ui-state-default ui-corner-all" id="create-user" value="Add New Color" onclick="document.location.href='index.php?p=color&a=add'" >
               		<div class="clear"></div>
				</div>
				<div class="clear"></div>
			</div>
...
$().ready(function() {
	...
	$("#FrmTestimonials").validate({
		...
		messages: {
			Testimonials: "Please enter your Color"
		}
	}
	});
});
	...
	<div id="page-content">
		<div id="page-content-wrapper">
			...
			<div class="content-box">
                    ...
                    <input type="hidden" name="p" value="color">
                            <?php if($a == "edit"){ ?>
                    	...
                    	<ul>
                        	<li>
                                ...
                                <input type="text" name="ColorName" id="ColorName"  class="field text small"  value="<?php echo viewtext($ColorName);?>" />
